...
export const Projects = () => {
  ...
  const handleQuickCreateClient = async (e) => {
    ...
    try {
      ...
      // Seamlessly reopen previous project modal
        ...
        <>
          ...
          {viewMode === 'operations' ? (
            <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e8e4dc', padding: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', width: '100%', alignItems: 'center', marginBottom: '1.25rem' }}>
              ...
              {operationsSubView === 'project' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {filteredProjects
                    .filter(p => selectedCategoryFilter === 'All' || p.projectCategory === selectedCategoryFilter || (selectedCategoryFilter === 'Architecture' && p.projectCategory?.includes('Architecture')))
                    .map((proj, index) => {
                ...
                <tbody>
                  {paginatedProjects.map((proj) => {
                    ...
                    return (
                      ...
                      >
                        <td style={{ padding: '12px 18px', textAlign: 'left', wordBreak: 'break-word' }}>
                          <div style={{ fontWeight: 600, color: '#1C1A17', fontSize: '13.5px' }}>{proj.projectName}</div>
                          <span style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '10px', color: '#8A857D', letterSpacing: '0.06em', background: '#F1EEE9', border: '1px solid #ECE9E4', borderRadius: '99px', padding: '2px 8px', display: 'inline-block', marginTop: '3px' }}>
                            {proj.projectCategory}
                          </span>
                        </td>
                        <td style={{ padding: '12px 18px', textAlign: 'center' }}>
                          <div style={{ fontWeight: 600, color: '#1C1A17', fontSize: '13px' }}>{clientName}</div>
                          <div style={{ fontSize: '11px', color: '#8A857D' }}>PM: {pmName}</div>
                        </td>
                        <td style={{ padding: '12px 18px', textAlign: 'center', fontFamily: 'Spline Sans Mono, monospace', fontSize: '12px', fontWeight: 600, color: '#8A857D' }}>
                          {proj.progressPercentage || 0}%
                        </td>
                        <td style={{ padding: '12px 18px', textAlign: 'center', fontFamily: 'Spline Sans Mono, monospace', fontSize: '13px', fontWeight: 600, color: '#1C1A17' }}>
                          ₹{Number(proj.budget || 0).toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 18px', textAlign: 'center' }}>
                          <span className={`badge ${proj.status === 'Completed' ? 'badge-success' : (proj.status === 'In Progress' ? 'badge-info' : (proj.status === 'Delayed' ? 'badge-danger' : 'badge-secondary'))}`}>
                            {proj.status}
                          </span>
                        </td>
                        <td style={{ padding: '12px 18px', textAlign: 'center' }}>
                          <div style={{ display: 'inline-flex', gap: '4px', justifyContent: 'center' }} onClick={(e) => e.stopPropagation()}>
                            <button onClick={(e) => { e.stopPropagation(); setSelectedProject(proj); setIsDetailModalOpen(true); }} className="btn btn-secondary" style={{ padding: '5px 10px', fontSize: '11.5px' }} title="View Project Details">
                              Details
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); handleOpenEditModal(proj); }} className="btn btn-secondary" style={{ padding: '5px 8px', color: '#1C1A17' }} title="Edit Project">
                              <Edit3 size={13} />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); setDeletingProjectId(proj._id); }} className="btn btn-secondary" style={{ padding: '5px 8px', color: '#C75B39', borderColor: '#EAD9CC' }} title="Delete Project">
                              <Trash2 size={13} />
                            </button>
                          </div>
                        </td>
      ...
      >
        <form onSubmit={handleCreateProject} noValidate>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '160px', overflowY: 'auto', border: '1px solid #ECE9E4', borderRadius: '8px', padding: '6px 8px', backgroundColor: '#FAF9F7' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '160px', overflowY: 'auto', border: '1px solid #ECE9E4', borderRadius: '8px', padding: '6px 8px', backgroundColor: '#FAF9F7' }}>
              {usersList.filter(u => u.status !== 'Inactive' && u.status !== 'Deactivated')
                ...
                .map(u => {
                  ...
                  return (
                    <label key={uId} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.825rem', cursor: 'pointer', padding: '0.35rem 0.5rem', borderRadius: '6px', backgroundColor: isSelected ? '#F1EEE9' : '#FFFFFF', border: isSelected ? '1px solid #C9C4BC' : '1px solid #ECE9E4', marginBottom: '4px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <input
                          type="checkbox"
                          checked={isSelected}
      ...
      {selectedProject && (
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ backgroundColor: '#FAF9F7', padding: '16px 18px', borderRadius: '12px', border: '1px solid #ECE9E4' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '10px', color: '#8A857D', letterSpacing: '0.06em', background: '#F1EEE9', border: '1px solid #ECE9E4', borderRadius: '99px', padding: '2px 8px' }}>
                  {selectedProject.projectCategory}
                </span>
                <span className={`badge ${selectedProject.status === 'Completed' ? 'badge-success' : (selectedProject.status === 'In Progress' ? 'badge-info' : (selectedProject.status === 'Delayed' ? 'badge-danger' : 'badge-secondary'))}`}>
                  {selectedProject.status}
                </span>
              </div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#1C1A17', letterSpacing: '-0.01em' }}>{selectedProject.projectName}</div>
              <div style={{ fontSize: '12px', color: '#8A857D', marginTop: '4px' }}>
                Client: <span style={{ color: '#1C1A17', fontWeight: 500 }}>{typeof selectedProject.client === 'object' ? (selectedProject.client?.companyName || selectedProject.client?.clientName) : 'Client'}</span> · PM: <span style={{ color: '#1C1A17', fontWeight: 500 }}>{typeof selectedProject.productionManager === 'object' ? (selectedProject.productionManager?.name || 'PM') : 'PM'}</span>
              </div>
            </div>

            {/* Key Metrics Grid */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px' }}>
              <div style={{ backgroundColor: '#FAF9F7', padding: '10px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em' }}>BUDGET</div>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '15px', fontWeight: 700, color: '#1C1A17', marginTop: '3px' }}>
                  ₹{Number(selectedProject.budget || 0).toLocaleString('en-IN')}
                </div>
              </div>
              <div style={{ backgroundColor: '#FAF9F7', padding: '10px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em' }}>PROGRESS</div>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '15px', fontWeight: 700, color: '#C75B39', marginTop: '3px' }}>
                  {selectedProject.progressPercentage || 0}%
                </div>
              </div>
              <div style={{ backgroundColor: '#FAF9F7', padding: '10px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em' }}>PRIORITY</div>
                <div style={{ fontSize: '13px', fontWeight: 600, color: selectedProject.priority === 'High' ? '#C75B39' : (selectedProject.priority === 'Medium' ? '#B45309' : '#2E7D4E'), marginTop: '3px' }}>
                  {selectedProject.priority || 'Medium'}
                </div>
              </div>
              <div style={{ backgroundColor: '#FAF9F7', padding: '10px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em' }}>TIMELINE</div>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '11px', fontWeight: 500, color: '#1C1A17', marginTop: '4px' }}>
            ...
            {selectedProject.assignedTeam && selectedProject.assignedTeam.length > 0 && (
              <div style={{ backgroundColor: '#FAF9F7', padding: '12px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em', marginBottom: '6px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {selectedProject.assignedTeam.map((m, idx) => {
                    ...
                    return (
                      <span key={idx} style={{ fontSize: '12px', backgroundColor: '#FFFFFF', border: '1px solid #ECE9E4', borderRadius: '99px', padding: '3px 10px', color: '#1C1A17', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                        👤 {name}
                      </span>
                    );
                  })}
            ...
            {selectedProject.notes && (
              <div style={{ backgroundColor: '#FAF9F7', padding: '12px 14px', borderRadius: '8px', border: '1px solid #ECE9E4' }}>
                <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '9.5px', color: '#8A857D', letterSpacing: '0.06em', marginBottom: '4px' }}>
                  SCOPE NOTES & DETAILS:
                </div>
                <div style={{ fontSize: '13px', color: '#55504A', whiteSpace: 'pre-line' }}>{selectedProject.notes}</div>
              </div>
            )}

            {/* Stages List */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ fontFamily: 'Spline Sans Mono, monospace', fontSize: '10px', color: '#8A857D', letterSpacing: '0.06em', marginTop: '4px' }}>PRODUCTION STAGES:</div>
              {selectedProject.stages && selectedProject.stages.length > 0 ? (
                selectedProject.stages.map((stage) => (
                  <div key={stage._id} style={{ border: '1px solid #ECE9E4', borderRadius: '10px', padding: '12px 16px', backgroundColor: '#FFFFFF' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      <div style={{ fontWeight: 600, fontSize: '13.5px', color: '#1C1A17' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <button
                          ...
                          style={{ background: 'none', border: 'none', color: '#C75B39', cursor: 'pointer', padding: '2px' }}
                    ...
                    {/* Sub-stages list */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', borderTop: '1px solid #F5F2ED', paddingTop: '8px' }}>
                      {stage.subStages && stage.subStages.length > 0 ? (
                        stage.subStages.map((sub) => (
                          <div key={sub._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', backgroundColor: '#FAF9F7', borderRadius: '6px', border: '1px solid #ECE9E4' }}>
                            <div>
                              <div style={{ fontSize: '12.5px', fontWeight: 500, color: '#1C1A17' }}>{sub.name}</div>
                              {sub.groupLabel && <div style={{ fontSize: '10.5px', color: '#8A857D' }}>Group: {sub.groupLabel}</div>}
                            </div>

                            {sub.status === 'Approved' ? (
                              <span style={{ fontSize: '11px', color: '#2E7D4E', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        <div style={{ fontSize: '11.5px', color: '#B5B0A8' }}>No sub-stages initialized</div>
                      )}
                    </div>
                  </div>
                ))
                ...
                <div style={{
                  ...
                  <div style={{ fontWeight: 600, color: '#1C1A17', marginBottom: '0.25rem', fontSize: '0.9rem' }}>No Production Stages Instantiated</div>
                  <p style={{ margin: 0, fontSize: '0.78rem', color: '#8A857D' }}>
                    This project does not have active workflow stages. Apply a workflow template or add a production stage.
                  </p>
                </div>
              )}
            </div>
